...
class DepartmentRepository:
    ...
    async def list_departments(self) -> Sequence[Department]:
        stmt = select(Department).options(selectinload(Department.category_links)).where(Department.is_active == True)
    ...
    async def find_department_for_category(self, category: str) -> Optional[Department]:
        stmt = (
            select(Department)
    ...
    async def list_wards(self) -> Sequence[Ward]:
        stmt = select(Ward).where(Ward.is_active == True).order_by(Ward.ward_number)
        result = await self.session.execute(stmt)
        return result.scalars().all()
